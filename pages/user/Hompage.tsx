'use client'
import { AllUserData, FriendSuggession, UserfindById, userHome, FollowUsers } from '@/lib/functions/user/route';
      ...
      <div className="hidden md:block w-96 ml-4 rounded-md mt-5 p-1 overflow-y-auto bg-white">
        <div className="top-8 right-28 bg-white rounded-lg shadow-md p-4">
          ...
          <ul className="space-y-4">
            {suggession.map((res: any, index: number) => {
              return (
                <li  key={index} className="cursor-pointer flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <img onClick={() => handleSuggession(res)} className="w-12 h-12 rounded-full" src={res?.image} alt="User avatar" />
                    <div>
                      <h3 onClick={() => handleSuggession(res)} className="font-semibold">{res?.username}</h3>
...
function LikedUser({ ShowLikers, singlePost }: any) {
  ...
  useEffect(()=>{
    ...
    const likedUserData = Promise.all( singlePost.likes.map(async (users:any)=>{
      const results = UserfindById(users)
      if(results){
        console.log(results)
        // setLikedUsers()
      }
        ...
        <ModalContent style={{ borderRadius: '20px' }} className="border w-96 border-gray-400 bg-slate-200 h-fit mt-28">
          {(ShowLikers) => (
            <>
              ...
              <ModalBody className="">
                {singlePost.likes.length>0 ? singlePost.likes.map((res: any, index: number) => {

                  <li onClick={() => handleSuggession(res)} key={index} className="cursor-pointer flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <img className="w-12 h-12 rounded-full" src={res?.image} alt="User avatar" />
                      <div>
                        <h3 className="font-semibold">{res?.username}</h3>

                      </div>
                    </div>
                    <button onClick={() => FollowUser(res)} className={res.followers.includes(user.user?._id) ?
