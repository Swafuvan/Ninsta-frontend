...
import PostLoadingSkeleton from "@/components/ui/postskelton";
...
const HomePage = () => {
  ...
  const [shuffledSuggestions, setShuffledSuggestions] = useState<User[]>([]);
  ...
  const maxStories =
    deviceType === "mobile" ? 3 : deviceType === "tablet" ? 4 : 7;
  ...
  useEffect(() => {
    if (suggession.length > 0) {
      const shuffled = [...suggession].sort(() => 0.5 - Math.random()).slice(0, 3);
    ...
    try {
      ...
      if (user && user.user) {
        ...
        if (index !== -1 && !updatedPost[index].likes.includes(user.user._id)) {
          ...
          updatedPost[index].likes = updatedPost[index].likes.filter(
  ...
  async function FollowUser(follower: any, index: number) {
    try {
      ...
      const userSuggession = suggession.findIndex(
        (data: any) => data._id === follower._id
      );
      console.log(userSuggession);
      if (
        followUser &&
        !updatedSuggession[userSuggession].followers.includes(
          user?.user?._id + ""
        )
      ) {
        updatedSuggession[userSuggession].followers.push(user?.user?._id + "");
      } else {
        updatedSuggession[userSuggession].followers = updatedSuggession[
          userSuggession
        ].followers.filter((item: any) => item !== user?.user?._id + "");
      }
      ...
      {user && (
        <>
          <div className="w-full ">
              ...
              <ul className="flex space-x-2 ">
                <li className="flex flex-col items-center space-y-1 ">
                  <div
                    className={
                      ownStoryData
                        ? "relative  bg-gradient-to-tr from-yellow-400 to-purple-600 z-0 p-1 rounded-full"
                        : "relative  bg-gradient-to-tr from-gray-400 to-white z-0 p-1 rounded-full"
                    }
                  >
                    <a
                      href="#"
                      className="block bg-white p-1 rounded-full transform transition z-0 hover:-rotate-6"
                    >
                      <img
                        onClick={() => {
                          setStoryUser(user?.user?._id + ""),
                            setShowStory(true);
                        }}
                        className="z-10 w-12 h-12 rounded-full"
                        src={user.user?.image + ""}
                        alt="cute User"
                      />
                    </a>
                ...
                {UserStory.length > 0
                  ? UserStory.slice(0, maxStories).map(
                      (story: any, index: number) => {
                        return (
                          ...
                          >
                            ...
                            <a
                              className="pl-1 w-20 truncate text-gray-800"
                              href="#"
                            >
                              {story.user.username}
                            </a>
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <img
                                  onClick={() => {
                                    setStoryUser(data?._id), setShowStory(true);
                                  }}
                                  className="w-12 h-12 rounded-full"
                                  src={data.image}
                                  alt="post"
                            ...
                            <a
                              className="pl-2 w-20 truncate text-gray-800 "
                              href="#"
                            >
                              {data.username}
                            </a>
            ...
            {addStory && (
              <StoryCreatePage userData={user.user} CloseStory={CloseStory} />
            )}
            {report && (
              <PostEditModal singlePost={singlePost} PostEdits={PostEdits} />
            )}
            {likedUser && (
              <LikedUser singlePost={singlePost} ShowLikers={ShowLikers} />
            )}
            {open && (
              <CommentsPage
                    ...
                    {Posts &&
                      Posts.map((item, idx) => {
                        return (
                          <div
                            key={idx}
                            className="bg-white border rounded-sm max-w-lg mb-4 justify-center"
                          >
                            <div className="flex items-center justify-between px-2 py-2">
                              <div className="flex ">
                                <img
                                  onClick={() =>
                                    handleSuggession(item.userDetails)
                                  }
                              ...
                              <div className="flex  justify-end">
                                <img
                                  ...
                                  onClick={() => {
                                    setReport(true), setSinglePost(item);
                                  }}
                                  src="/icons8.png"
                                  alt="image"
                                />
                            ...
                            <div
                              onClick={() => {
                                setSinglePost(item), setlikedUser(true);
                              }}
                              className="cursor-pointer font-semibold text-sm mx-4 mt-5 mb-4"
                            >
                            ...
                            <div className="text-xs text-gray-500 mx-4 mb-2">
          ...
          <div className="hidden md:block w-96 ml-4 rounded-md mt-5 p-1 overflow-y-auto bg-white">
            <div className="top-8 right-28 bg-white rounded-lg shadow-md p-4">
              ...
              {shuffledSuggestions.map((res: any, index: number) => {
                    return (
                      <li
                        key={index}
                      ...
                      >
                        ...
                        <button
                          onClick={() => FollowUser(res, index)}
                          className={
                            res.followers.includes(user.user?._id)
    ...
    <>
      ...
      >
        <ModalContent
          ...
          className="border w-96 border-gray-400 bg-slate-200 h-fit mt-28"
        >
          {(ShowLikers) => (
            <>
              ...
              <ModalBody className="">
                {LikedUsers.length > 0 ? (
                  LikedUsers.map((res: any, index: number) => {
                    // Check if the current user liked the post
                    const isLikedByCurrentUser = res?.userDetail._id === user.user?._id;
                      ...
                      >
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            {/* If liked by the current user, show "You" */}
                            {isLikedByCurrentUser && (
                              <p className="text-sm text-gray-600">You</p>
                            )}
                          </div>
                        </div>
                        {/* If not the current user, show the Follow/Unfollow button */}
                        {!isLikedByCurrentUser && (
                          <button
                            onClick={() => FollowUser(res)}
                            className={
                              res?.userDetail?.followers.includes(user.user?._id)
                                ? "bg-slate-200 text-black px-3 py-2 rounded-full"
                                : "bg-blue-500 text-white px-4 py-2 rounded-full"
                            }
                          >
