...
export default function ModalPage({ handleDrawerOpen }: any) {
  ...
  async function handleFiles(e: any) {
    ...
    const UploadedData = await postUpload(files, text);
            ...
            <div>
              ...
              <form onSubmit={(e) => handleFiles(e)}>
                <ModalBody className="flex justify-between items-center mt-2 p-10">
                  ...
                  <input onClick={(e) => e.stopPropagation()} onChange={onChange} type="file" multiple name="PostUpload" id="PostUpload" />
                  <div className="flex flex-wrap gap-4">
                    {images.length > 0 && images.map((item: any, idx: number) => (
                ...
                <ModalFooter>
                  <Button className="bg-slate-400 rounded-full" variant="light" onPress={handleDrawerOpen}>
                    Close
                  </Button>
                  {images.length > 0 && (
                    <Button
