...
function PostReports() {
    ...
    useEffect(() => {
        const postRepo = UserPostReport().then(async (userReport) => {
            ...
            // setReport(userReport?.userReport)
            // const userDetails = userReport?.data?.userReport.map(async(user:any)=>{
            //     return await UserFindbyId(user?.reportedBy)
            // })
            // Promise.all(userDetails).then((userData:any) => {
            //     console.log(userData)
            //     setUserData(userData)
            // })
            // const data = await Promise.all(userReport?.data?.userReport.map((async (item: any, index: number) => {
            //     userReport.data.userReport[index].userDetails = await UserFindbyId(item?.reportedBy)
            //     return userReport.data.userReport[index]
            // })))
            // console.log(data)
            // setReport(data)
        })
        ...
        <div className='mr-3'>
            <div className="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
                <div className="w-full mb-1">
                    <div className="mb-4">
                        <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">All Reports</h1>
                    </div>
