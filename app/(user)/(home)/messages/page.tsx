...
function MessagesPage() {
    ...
    const filterUsers = (userdata: any) => {
        const newusers = [userdata, ...noMessaged].filter((usr, idx, arr) => arr.indexOf(usr) === idx)
        setNoMessaged(newusers)
    }
    ...
    return (
        <div className="bg-gray-50 h-screen w-full">
            ...
            {!messageSide ?
                ...
                :
                <MessagePage userDetails={messageSide} filter={filterUsers} toggleMessageSide={toggleMessageSide} />
            }
        </div>
        </div >
    );
